...
1. O processo `fato_vendas.py` inicia e tenta ler a data da última execução bem-sucedida a partir de um arquivo de configuração no MinIO (`s3a://gold/checkpoints/fato_vendas/_checkpoint.json`).
2. Se o arquivo existir, o timestamp contido ali é a variável `last_run`.
...
3. **Somente após a gravação Delta ser concluída** com sucesso é que o nosso script atualiza o arquivo `.json` do Checkpoint para a nova data (o maior valor encontrado na leitura atual).
